...
function parseRobustDate(dateStr, timeStr) {
  if (!dateStr) return 0;
  if (timeStr) timeStr = timeStr.toString().trim().padStart(5, '0');
...
function navigateTo(pageId) {
  ...
  document.querySelectorAll('.nav-item').forEach(function(item) {
    item.classList.remove('bg-secondary-fixed/10', 'text-secondary-fixed');
  ...
  if (activeNav) {
    ...
    activeNav.classList.add('bg-secondary-fixed/10', 'text-secondary-fixed');
  }
...
function renderDashboardTable() {
  ...
  sortedReports.forEach(function(r) {
      ...
      if (Array.isArray(photos) && photos.length > 0) {
        ...
        photoHtml = `<img src="${thumbUrl}" class="w-24 h-16 rounded object-cover border border-surface-variant" alt="Foto" onerror="this.src='${fullUrl}'; this.onerror=null;">`;
      } else {
        photoHtml = `<div class="w-24 h-16 rounded bg-surface flex items-center justify-center text-on-surface-variant/30 border border-surface-variant"><span class="material-symbols-outlined text-[24px]">hide_image</span></div>`;
      }
      
      // Judul RHK
      var idText = r.IdRHK || r.JenisRHK || '';
      var angkaRHK = idText.replace(/\D/g, '') || '?';
      var judulHTML = `<div class="font-bold text-on-surface mb-0.5 text-sm leading-tight line-clamp-2" title="${r.JenisRHK || ''}">(RHK-${angkaRHK}) ${r.JenisRHK || '-'}</div>`;
      ...
      tableHtml += `
        <tr class="hover:bg-primary/5 transition-colors group cursor-pointer" onclick="previewPdf('${r.ReportId}')">
          ...
          <td class="px-3 py-3 align-top min-w-[150px]">
            ${judulHTML}
            <div class="text-[10px] text-primary/80 font-medium leading-tight mt-1 line-clamp-2" title="${r.RencanaAksi || ''}">
