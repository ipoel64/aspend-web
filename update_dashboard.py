...
new_render_dashboard = """function renderDashboardTable() {
  ...
  sortedReports.forEach(function(r) {
      ...
      let reportDateStr = r.Tanggal || 'Tidak Diketahui';
      if (reportDateStr !== lastDateGroup) {
        lastDateGroup = reportDateStr;
        tableHtml += `
          <tr class="bg-surface-container-lowest">
            <td colspan="4" class="px-4 py-2">
              <div class="flex items-center gap-3">
                <div class="h-px bg-outline-variant flex-grow"></div>
                <span class="text-[10px] font-bold text-primary uppercase tracking-widest bg-primary/10 px-2 py-0.5 rounded">${reportDateStr}</span>
                <div class="h-px bg-outline-variant flex-grow"></div>
              </div>
            </td>
          </tr>
      ...
      // Photo rendering with Fallback handling
      ...
      if (Array.isArray(photos) && photos.length > 0) {
        photoHtml = `<img src="https://drive.google.com/uc?id=${photos[0]}&export=view" class="w-10 h-10 rounded object-cover cursor-pointer hover:opacity-80 border border-surface-variant" onclick="window.open('https://drive.google.com/uc?id=${photos[0]}', '_blank')" alt="Foto">`;
      } else {
      ...
      // Membuang Lokasi & Status, Menyisipkan (RHK-...) ke Judul
      var judulHTML = `<div class="font-bold text-on-surface mb-1">(RHK-${r.JenisRHK || '?'}) ${r.RencanaAksi || '-'}</div>`;
      var uraianHTML = r.Uraian ? `<div class="text-xs text-on-surface-variant mt-1 line-clamp-2">${r.Uraian}</div>` : '';

      tableHtml += `
        <tr class="hover:bg-surface-container-lowest transition-colors group">
          <td class="px-4 py-3 align-top whitespace-nowrap">
            <div class="font-medium text-on-surface">${r.Tanggal}</div>
...
if old_match:
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("renderDashboardTable updated successfully.")
else:
